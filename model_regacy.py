...
class NaiveAE(VAE):
    ...
    def __init__(
        ...
        self.encoder.append(
            torch.nn.Sequential(
                ...
                torch.nn.LeakyReLU(),
                torch.nn.Linear(latent_channel * 2, latent_channel * 1),
            )
        )
    ...
    def encode(self, input):
        ...
        return z, 0.0 # prevent var from being negative

    def decode(self, input):
        return self.decoder(input)
    
    def forward(self, input, latent_recon=False, latent_rand_sampling=False):
        if latent_recon:
            return self.forward_Ex(input) # should be equal to lrvae source
